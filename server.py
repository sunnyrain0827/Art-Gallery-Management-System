...
def deleteexhibition():
  cursor2 = g.conn.execute("SELET enddate FROM exhibition")
  data = []
  for result in cursor2:
    data.append({"enddate":result[0]})
    cursor2.close
  ...
  if request.method == 'POST':
      ...
      cursor1 = g.conn.execute(sql_delete_query, e_id)
      cursor1.close

      return redirect(url_for('deleteexhibition'))
  else:
      return render_template("deleteexhibition.html", data = data)
